...
const STATUS_CONTENTS: Record<`${ActionStatus}`, React.ReactNode> = {
  queued: <Status status="queued" statusHumanReadable="Queued" statusReasonHumanReadable="" />,
  pending: <Status status="pending" statusHumanReadable="Pending" statusReasonHumanReadable="" />,
  "in-progress": (
    <Status status="in-progress" statusHumanReadable="In Progress" statusReasonHumanReadable="" />
  ),
  completed: (
    <Status status="completed" statusHumanReadable="Completed" statusReasonHumanReadable="" />
  ),
  failed: <Status status="failed" statusHumanReadable="Failed" statusReasonHumanReadable="" />,
  waiting: <Status status="waiting" statusHumanReadable="Waiting" statusReasonHumanReadable="" />,
  skipped: <Status status="skipped" statusHumanReadable="Skipped" statusReasonHumanReadable="" />,
};
  ...
  const columns: Column<MediumRow>[] = [
    ...
    },
    {
      ...
      Header: ({ rows: tableRows }) => {
        ...
        const isChecked = rowIds.length > 0 && rowIds.every((id) => selected.includes(id));
      ...
      Cell: ({ row }) => {
        ...
        return (
          <CheckBox
            ...
            onClick={() =>
              setSelected(
                isChecked ? selected.filter((selectedId) => selectedId !== id) : [...selected, id],
              )
            }
          />
        );
