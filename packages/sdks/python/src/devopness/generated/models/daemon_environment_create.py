...
class DaemonEnvironmentCreate(DevopnessBaseModel):
    """
    ...
    command: StrictStr = Field(
        ...
        json_schema_extra={
            "examples": [
                "/usr/bin/example --parameter=%(ENV_VAR_TO_GET_PARAM_VALUE_FROM)s"
            ]
        },
    )
    ...
    working_directory: StrictStr | None = Field(
        ...
        json_schema_extra={"examples": ["relative-path/within/application/folder"]},
    )
    run_as_user: StrictStr = Field(
        ...
        json_schema_extra={"examples": ["root-of-all-evil"]},
    )
    name: StrictStr = Field(
        ...
        json_schema_extra={"examples": ["we-all-have-daemons-inside"]},
    )
...
class DaemonEnvironmentCreatePlain(TypedDict, total=False):
    """
