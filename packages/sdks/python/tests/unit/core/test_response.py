...
def build_response(
    ...
    state: DevopnessClientState | None = None,
) -> Mock:
    ...
    request.extensions[DEVOPNESS_CLIENT_STATE_EXTENSION_KEY] = state
...
class TestDevopnessResponse(unittest.TestCase):
    def setUp(self) -> None:
        self.strict_state = DevopnessClientState(
            config=DevopnessClientConfig(strict_validation_mode=True)
        )
        self.lenient_state = DevopnessClientState(
    ...
    def test_devopness_response_with_dict(self) -> None:
        response: DevopnessResponse[DummyModel] = DevopnessResponse(
            build_response({"id": 123}, state=self.strict_state),
            DummyModel,
        )
    ...
    def test_devopness_response_with_list(self) -> None:
        response: DevopnessResponse[list[DummyModel]] = DevopnessResponse(
            build_response([{"id": 123}, {"id": 456}], state=self.strict_state),
            list[DummyModel],
        )
    ...
    def test_devopness_response_with_str(self) -> None:
        response: DevopnessResponse[str] = DevopnessResponse(
            build_response(b"Hello", state=self.strict_state),
            str,
        )
    ...
    def test_devopness_response_with_int(self) -> None:
        response: DevopnessResponse[int] = DevopnessResponse(
            build_response(b"42", state=self.strict_state),
            int,
        )
    ...
    def test_devopness_response_with_empty_body(self) -> None:
        response: DevopnessResponse[None] = DevopnessResponse(
            build_response(b"", state=self.strict_state),
        )
    ...
    def test_devopness_response_with_no_pagination(self) -> None:
        response: DevopnessResponse[None] = DevopnessResponse(
            build_response(state=self.strict_state),
        )
    ...
    def test_devopness_response_strict_validation_raises(self) -> None:
        with self.assertRaises(ValidationError):
            DevopnessResponse(
                build_response({"id": "not-an-int"}, state=self.strict_state),
                DummyModel,
            )
    ...
    def test_devopness_response_non_strict_validation_warns_in_debug_mode(
        ...
        debug_state = DevopnessClientState(
            config=DevopnessClientConfig(
                strict_validation_mode=False,
                debug=True,
            )
...
class TestDevopnessResponseAsync(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_devopness_response_async_without_originating_request(
        ...
        parsed_response_value: DevopnessResponse[DummyModel]
        parsed_response_value = await DevopnessResponse.from_async(
            response,
            DummyModel,
        )

        assert isinstance(parsed_response_value.data, DummyModel)
        assert parsed_response_value.data.id == 123
