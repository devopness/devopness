...
test("base URL is configurable on initialization", () => {
  ...
  expect(apiClient).toHaveProperty("networks");
  ...
  expect(apiClient).toHaveProperty("resourceLinks");
  ...
  expect(apiClient).toHaveProperty("subnets");
...
test("expired access_token should trigger callback function", async () => {
  const mockRefreshTokenCallback = vi.fn((token: string) => token);

  const apiClient = new DevopnessApiClient();
  apiClient.accessToken =
    "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..GirkUv0UI1kUORKpcWSrSKsfbM-s5Hwd1LSlDpExgbg";
