...
- "Add a `.env` file to the `acme-api` application in the Production environment with `DATABASE_URL` and `API_KEY`."
- "Create a configuration file `config.yaml` for `acme-web` in Staging and set environment variables from this list."
