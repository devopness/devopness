...
1. On Devopness, copy the IP Address of the server you want to connect
    > Follow the [`/docs/servers/find-server-ip-address`](/docs/servers/find-server-ip-address) guide
1. On your local machine, in a terminal window, use the `ssh` command to connect to the server
    ...
    > If you receive a `Connection refused` error, please follow the [`/docs/ssh-keys/add-ssh-key`](/docs/ssh-keys/add-ssh-key) guide
