 ...
 */
export const formatPermissionHumanReadable = (unformattedPermission: string) => {
  ...
  const validPermissionFormat = /^([a-z-]+:[a-z_]+)$/;
  ...
  const isPermissionFormatValid = trimmedPermission.match(validPermissionFormat);
  if (!isPermissionFormatValid) return defaultValue;

  const staticResource = staticPermissions.find(
    ({ resource_type: static_resource_type }) => static_resource_type == resource_type,
  );
